import Link from 'next/link';

export default function Footer() {
  return (
    <footer className="bg-verde-oscuro text-white">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Contacto */}
          <div>
            <h3 className="text-xl font-bold mb-4 text-beige">Contacto</h3>
            <p className="mb-2">Ayuntamiento de Santibáñez el Bajo</p>
            <p className="mb-2">Pl. España, 1</p>
            <p className="mb-2">10666 Santibáñez el Bajo, Cáceres</p>
            <p className="mb-2">
              <a href="tel:927670059" className="hover:text-verde-claro transition-colors">
                Tel: 927 67 00 59
              </a>
            </p>
            <p>
              <a href="tel:669297211" className="hover:text-verde-claro transition-colors">
                Tel: 669 29 72 11
              </a>
            </p>
            <p className="mt-2">
              <a
                href="mailto:info@crosslaslagunas.com"
                className="hover:text-verde-claro transition-colors"
              >
                info@crosslaslagunas.com
              </a>
            </p>
          </div>

          {/* Enlaces */}
          <div>
            <h3 className="text-xl font-bold mb-4 text-beige">Enlaces</h3>
            <ul className="space-y-2">
              <li>
                <Link href="/categorias" className="hover:text-verde-claro transition-colors">
                  Categorías y Distancias
                </Link>
              </li>
              <li>
                <Link href="/inscripcion" className="hover:text-verde-claro transition-colors">
                  Inscripción
                </Link>
              </li>
              <li>
                <Link href="/informacion" className="hover:text-verde-claro transition-colors">
                  Información Útil
                </Link>
              </li>
              <li>
                <Link href="/contacto" className="hover:text-verde-claro transition-colors">
                  Contacto
                </Link>
              </li>
            </ul>
          </div>

          {/* Redes Sociales */}
          <div>
            <h3 className="text-xl font-bold mb-4 text-beige">Síguenos</h3>
            <div className="flex space-x-4">
              <a
                href="https://facebook.com"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-verde-claro transition-colors"
                aria-label="Facebook"
              >
                <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
            </div>
          </div>
        </div>

        <div className="border-t border-verde-claro mt-8 pt-8 text-center">
          <p>&copy; 2025 Cross Las Lagunas - Todos los derechos reservados</p>
          <div className="mt-2 space-x-4">
            <Link href="/aviso-legal" className="text-sm hover:text-verde-claro transition-colors">
              Aviso Legal
            </Link>
            <Link href="/privacidad" className="text-sm hover:text-verde-claro transition-colors">
              Política de Privacidad
            </Link>
            <Link href="/cookies" className="text-sm hover:text-verde-claro transition-colors">
              Política de Cookies
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
